...
describe('MaintenanceService scheduling', () => {
  ...
  it('only schedules new work orders when past lead time and no open orders', async () => {
    const rule = createRule({ nextDueDate: '2025-02-01' });
    const closedOrder = createWorkOrder({ state: 'done' });

    (storageProvider.getWorkOrders as unknown as Mock).mockResolvedValue([closedOrder]);

    const shouldCreate = await (service as unknown as { shouldCreateWorkOrderForRule: (r: MaintenanceRule, date: Date) => Promise<boolean> }).shouldCreateWorkOrderForRule(
      rule,
    ...
    const openOrder = createWorkOrder({ state: 'in-progress' });
    (storageProvider.getWorkOrders as unknown as Mock).mockResolvedValue([openOrder]);

    const blocked = await (service as unknown as { shouldCreateWorkOrderForRule: (r: MaintenanceRule, date: Date) => Promise<boolean> }).shouldCreateWorkOrderForRule(
      rule,
      new Date('2025-01-25T00:00:00Z'),
    );
